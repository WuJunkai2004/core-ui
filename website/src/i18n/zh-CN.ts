export default {
  ...
  "home.subtitle": "Direct2D 硬件加速渲染、Fluent 2 设计系统、原生控件、Vue 3 风格 .uix 单文件组件和纯 C API — 全部打包在一个 DLL 中。",
  ...
  "feature.api.desc": "基于不透明句柄的纯 C 函数。可从任何语言调用 — 无 C++ 类型暴露。",
  "feature.controls.title": "内置控件",
  "feature.controls.desc": "从 Button、CheckBox、Slider 到 ImageView、TabControl 和声明式菜单。可直接用于生产应用。",
  ...
  "gs.requirements": "Core UI 需要 Windows 10 或更高版本、CMake 3.20+，以及支持 C++17 的 MSVC 2019+ 或 clang-cl。不支持 MinGW。",
  "gs.buildTitle": "从源码构建",
  "gs.cmakeTitle": "CMake 集成",
  "gs.cmakeDesc": "在你的 CMake 项目中链接 core-ui：",
  "gs.minimalTitle": "最小示例 — C API",
  "gs.minimalDesc": "使用纯 C API 创建一个包含标签和按钮的窗口。公共对象都是不透明 uint64_t 句柄 — 不暴露 C++ 类型。",
  ...
  // Controls
  "controls.title": "控件",
  "controls.subtitle": "内置控件和 .uix 标签覆盖容器、输入、显示和导航。所有控件遵循 Fluent 2 设计规范，主题感知渲染。",
  ...
  "markup.directivesDesc": "{{ expr }} 文本插值、:attr=\"expr\" 属性绑定、@event=\"...\" 事件、v-if 条件挂载、v-show 可见性切换、v-for 列表（带 :key 走 keyed diff）、v-model 双向绑定。所有表达式按 JS 语义求值，访问 this.xxx 写成裸 xxx。",
  ...
  "markup.lifecycleTitle": "生命周期 hooks",
  ...
  "markup.reactiveMenuTitle": "反应式菜单",
  "markup.reactiveMenuDesc": "<menu> 全反应式：menuitem 用 v-for 跟着 data 数组渲染，v-if 控制是否显示，:disabled 跟状态联动。右键弹出永远读最新值，不需要手动重建菜单。rclick dispatch 改成 deepest-match，嵌套 <menu trigger> 时子 widget 的 menu 不再被祖先抢走。注意 menuitem 是 widget 模板，子元素必须用 <label> 包裹文本（不能直接 <menuitem>文本</menuitem>）。",
  ...
  // Layout
  "layout.title": "布局系统",
  "layout.subtitle": "Core UI 用 CSS flex 子集做布局：div + flex-direction（默认 column，行就 row）+ gap + align/justify。ScrollView 也提供 C API widget。",
  ...
  "layout.splitViewDesc": "分栏导航建议用 row 容器组合：固定宽侧栏 + flex: 1 内容区。",
  "layout.splitterDesc": "当前没有导出的公共 Splitter widget；需要拖拽分隔时可先用固定/flex 面板或 custom widget 实现。",
  ...
  // Debug page
  "debug.title": "调试与自动化",
  "debug.subtitle": "一整套事件注入 API —— 无需真实鼠标键盘即可驱动任意控件，面向自动化测试、AI 代理操作 UI、脚本化回归等场景。",
  ...
  "ctrl.Dialog.desc": "模态消息框，支持自定义按钮和键盘行为",
