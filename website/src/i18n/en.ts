export default {
  ...
  "home.subtitle": "Direct2D hardware-accelerated rendering, Fluent 2 design system, native controls, Vue 3 style .uix single-file components, and a pure C API — all in a single DLL.",
  ...
  "feature.api.desc": "Opaque handle-based pure C functions. Callable from any language — no C++ exposure.",
  "feature.controls.title": "Built-in Controls",
  "feature.controls.desc": "From Button, CheckBox, Slider to ImageView, TabControl, and declarative menus. Ready for production apps.",
  ...
  "gs.requirements": "Core UI requires Windows 10 or later, CMake 3.20+, and MSVC 2019+ or clang-cl with C++17 support. MinGW is not supported.",
  ...
  "gs.minimalDesc": "Create a window with a label and a button using the pure C API. Public objects are opaque uint64_t handles — no C++ types leak through.",
  ...
  "controls.subtitle": "Built-in controls and .uix tags covering containers, input, display, and navigation. All controls follow Fluent 2 design specifications with theme-aware rendering.",
  ...
  "markup.directivesDesc": "{{ expr }} text interpolation, :attr=\"expr\" attribute binding, @event=\"...\" handler, v-if for conditional mount, v-show for visibility toggles, v-for with :key for keyed list diff, and v-model for two-way binding. Expressions evaluate as JS — bare names resolve as this.xxx.",
  ...
  "markup.lifecycleTitle": "Lifecycle Hooks",
  ...
  "markup.reactiveMenuTitle": "Reactive Menus",
  "markup.reactiveMenuDesc": "<menu> is now fully reactive: menuitem uses v-for to bind to data arrays, v-if to gate visibility, and :disabled to flow state. Right-click pop-up reads the latest values every time — no manual rebuild needed. rclick dispatch became deepest-match, so nested <menu trigger> blocks no longer let an ancestor steal the child widget's menu. Note: menuitem is a widget template — wrap text content in <label> (no bare <menuitem>text</menuitem>).",
  ...
  "layout.subtitle": "Core UI lays out via a CSS flex subset: div + flex-direction (column by default, row otherwise) + gap + align/justify. ScrollView also ships as a C API widget.",
  ...
  "layout.splitViewDesc": "For a navigation pane, compose a row container with a fixed-width sidebar and a flex: 1 content area.",
  "layout.splitterDesc": "A public Splitter widget is not exported yet; use fixed/flex panels or a custom widget if you need a draggable divider.",
  ...
  "debug.subtitle": "A full event-injection API for automated tests, AI agents driving UIs, and scripted regressions. Every interactive control can be triggered programmatically without a real mouse or keyboard.",
  ...
  "ctrl.Dialog.desc": "Modal message box with custom buttons and keyboard behavior",
